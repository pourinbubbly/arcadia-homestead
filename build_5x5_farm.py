...
<head>
  ...
  <style>
    ...
    *, html, body {
    ...
    .features-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 36px; opacity: 1; margin-left: 50px; }
...
<body>
  ...
  <div id="landing-view">
    ...
    <section id="specs">
      ...
      <div class="specs-wrapper">
        <!-- RABBIT BUGS BUNNY BLOCK PUSHING CANVAS -->
  ...
  <script>
    ...
    /* 16-BIT STARDEW BUGS BUNNY HAND-DRAWN CANVAS PIXEL SPRITE RENDERER */
    function draw16BitBugsBunny(ctx, x, y, pose, scale = 4.5) {
      // 16x24 Pixel Art Matrix Definition for BUGS BUNNY
      // G=Gray(#9ca3af), W=White(#ffffff), P=Pink(#f472b6), B=Blue(#3b82f6), O=Orange(#f97316), K=Black(#1a0f0a)
      ctx.save();
      ...
      ctx.fillRect(-2*s, -26*s, 4*s, 12*s); ctx.fillRect(6*s, -28*s, 4*s, 14*s);
      ctx.fillStyle = '#9ca3af';
      ctx.fillRect(-1*s, -25*s, 2*s, 10*s); ctx.fillRect(7*s, -27*s, 2*s, 12*s);
      ctx.fillStyle = '#f472b6';
      ctx.fillRect(0*s, -23*s, 1*s, 6*s); ctx.fillRect(8*s, -25*s, 1*s, 8*s);

      // Draw Head & Face
      ctx.fillStyle = '#9ca3af'; ctx.fillRect(-4*s, -14*s, 14*s, 10*s);
      ctx.fillStyle = '#ffffff'; ctx.fillRect(-2*s, -10*s, 10*s, 6*s); // Muzzle
      ctx.fillStyle = '#1a0f0a'; ctx.fillRect(2*s, -13*s, 2*s, 3*s); ctx.fillRect(6*s, -13*s, 2*s, 3*s); // Eyes
      ctx.fillStyle = '#ffffff'; ctx.fillRect(3*s, -13*s, 1*s, 1*s); ctx.fillRect(7*s, -13*s, 1*s, 1*s); // Highlights
      ctx.fillStyle = '#f472b6'; ctx.fillRect(3*s, -9*s, 3*s, 2*s); // Nose

      if (pose === 'walk') {
        // Body with Farmer Blue Overalls
        ctx.fillStyle = '#3b82f6'; ctx.fillRect(-4*s, -4*s, 12*s, 10*s);
        ctx.fillStyle = '#fde047'; ctx.fillRect(-1*s, -3*s, 2*s, 2*s); ctx.fillRect(5*s, -3*s, 2*s, 2*s); // Gold Buttons
        ctx.fillStyle = '#ffffff'; ctx.fillRect(-3*s, -2*s, 10*s, 5*s); // White Chest

        // Walking Legs
        const legSwing = Math.floor(Math.sin(Date.now() * 0.01) * 4);
        ctx.fillStyle = '#9ca3af';
        ctx.fillRect(-3*s + legSwing*s, 6*s, 3*s, 7*s);
        ctx.fillRect(4*s - legSwing*s, 6*s, 3*s, 7*s);
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(-5*s + legSwing*s, 11*s, 6*s, 3*s);
        ctx.fillRect(3*s - legSwing*s, 11*s, 6*s, 3*s);

        // Walking Arms holding carrot
        ctx.fillStyle = '#ffffff'; ctx.fillRect(8*s, -2*s, 4*s, 4*s);
        // Carrot
        ctx.fillStyle = '#f97316'; ctx.fillRect(11*s, -4*s, 5*s, 3*s);
        ctx.fillStyle = '#22c55e'; ctx.fillRect(16*s, -5*s, 3*s, 4*s);

      } else if (pose === 'lean_munch') {
        // Bugs Bunny Leaning against LEFT side of Card 1!
        ctx.fillStyle = '#3b82f6'; ctx.fillRect(-4*s, -4*s, 12*s, 10*s);
        ctx.fillStyle = '#fde047'; ctx.fillRect(-1*s, -3*s, 2*s, 2*s); ctx.fillRect(5*s, -3*s, 2*s, 2*s);
        ctx.fillStyle = '#ffffff'; ctx.fillRect(-3*s, -2*s, 10*s, 5*s);

        // Crossed Leaning Legs
        ctx.fillStyle = '#9ca3af';
        ctx.fillRect(-1*s, 6*s, 3*s, 8*s); ctx.fillRect(2*s, 6*s, 3*s, 8*s);
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(-3*s, 12*s, 6*s, 3*s); ctx.fillRect(1*s, 12*s, 6*s, 3*s);

        // Left Arm Leaning Elbow against Card Frame on Right!
        ctx.fillStyle = '#ffffff'; ctx.fillRect(8*s, -3*s, 6*s, 3*s); ctx.fillRect(12*s, -6*s, 3*s, 6*s);

        // Right Hand Holding Carrot to Mouth & Munching!
        const munch = Math.floor(Math.sin(Date.now() * 0.02) * 2);
        ctx.fillStyle = '#ffffff'; ctx.fillRect(-6*s, -6*s + munch*s, 4*s, 4*s);
        // Carrot
        ctx.fillStyle = '#f97316'; ctx.fillRect(-10*s, -8*s + munch*s, 6*s, 4*s);
        ctx.fillStyle = '#22c55e'; ctx.fillRect(-14*s, -9*s + munch*s, 4*s, 5*s);
    ...
    /* BUGS BUNNY LEANING & CARROT MUNCHING ENGINE FOR SPECS SECTION */
    ...
    function triggerRabbitSpecsAnimation() {
      ...
      canvas.width = wrapper.clientWidth + 140;
      canvas.height = 460;
      const ctx = canvas.getContext('2d');
      ctx.imageSmoothingEnabled = false;

      // Target position: LEANING ON THE LEFT SIDE OF CARD 1!
      const card1 = document.getElementById('card-spec-1');
      const card1Left = card1 ? card1.offsetLeft : 60;
      const leanTargetX = card1Left - 10;

      let rabbitX = -100;
      let phase = 'walking'; // 'walking' -> 'leaning'

      function animateBugsBunnyLoop() {
        ctx.clearRect(0, 0, canvas.width, canvas.height);

        if (phase === 'walking') {
          rabbitX += 7;
          if (rabbitX >= leanTargetX) {
            rabbitX = leanTargetX;
            phase = 'leaning';
          }
          draw16BitBugsBunny(ctx, rabbitX, 190, 'walk', 4.5);
        } else if (phase === 'leaning') {
          draw16BitBugsBunny(ctx, leanTargetX, 190, 'lean_munch', 4.5);
        }

        requestAnimationFrame(animateBugsBunnyLoop);
      }

      animateBugsBunnyLoop();
    }
...
print("HAND-DRAWN 16-BIT BUGS BUNNY LEANING ON LEFT SIDE OF CARD 1 UPDATED SUCCESSFULLY!")
